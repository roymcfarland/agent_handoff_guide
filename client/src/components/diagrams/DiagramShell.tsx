...
export function DiagramShell({
  ...
  return (
    ...
    >
      <figcaption className="flex flex-col gap-1 border-b border-border px-5 py-3 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
        <div>
          <span className="font-mono text-[11px] font-bold uppercase tracking-widest text-muted-foreground">
            {figure} — {title}
          </span>
          {caption && (
            <p className="mt-1 text-xs leading-snug text-muted-foreground sm:mt-0 sm:max-w-xl">
              {caption}
            </p>
          )}
        </div>
      ...
      <div
        className={`bg-background px-3 py-6 sm:px-6 sm:py-8 ${contentClassName ?? ""}`}
      >
        {children}
      </div>
      {footer ? (
